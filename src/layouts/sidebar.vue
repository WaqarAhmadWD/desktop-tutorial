<template>
  <div
    id="application-sidebar"
    class="fixed inset-y-0 start-0 z-[60] w-64 transform overflow-y-auto border-e border-default-200 bg-white transition-all duration-300 hs-overlay-open:translate-x-0 lg:bottom-0 lg:right-auto lg:block lg:translate-x-0 hide-in-print"
    :class="{ block: toggleMenuMobile, hidden: !toggleMenuMobile }"
  >
    <div class="flex lg:block items-center justify-between">
      <button
        type="button"
        class="text-default-500 hover:text-default-600 lg:hidden block mx-6 my-4"
        data-hs-overlay="#application-sidebar"
        aria-controls="application-sidebar"
        aria-label="Toggle navigation"
        @click="toggleMenuMobileFun"
      >
        <svg
          stroke="currentColor"
          fill="none"
          stroke-width="2"
          viewBox="0 0 24 24"
          stroke-linecap="round"
          stroke-linejoin="round"
          height="24"
          width="24"
          xmlns="http://www.w3.org/2000/svg"
        >
          <line x1="3" x2="21" y1="6" y2="6"></line>
          <line x1="3" x2="21" y1="12" y2="12"></line>
          <line x1="3" x2="21" y1="18" y2="18"></line>
        </svg>
      </button>
      <div
        class="sticky top-0 flex h-18 items-center justify-center border-b border-dashed border-default-200 px-6 py-2"
      >
        <a href="/yum_b/home"
          ><img
            src="data:image/png;base64,"
            alt="logo"
            class="flex h-10 dark:hidden" /><img
            src="data:image/png;base64,"
            alt="logo"
            class="hidden h-10 dark:flex"
        /></a>
      </div>
    </div>
    <div
      data-simplebar="init"
      class="h-[calc(100%-390px)] simplebar-scrollable-y"
    >
      <div class="simplebar-wrapper" style="margin: 0px">
        <div class="simplebar-height-auto-observer-wrapper">
          <div class="simplebar-height-auto-observer"></div>
        </div>
        <div class="simplebar-mask">
          <div class="simplebar-offset" style="right: 0px; bottom: 0px">
            <div
              class="simplebar-content-wrapper"
              tabindex="0"
              role="region"
              aria-label="scrollable content"
              style="height: auto; overflow: hidden scroll"
            >
              <div class="simplebar-content" style="padding: 0px">
                <ul
                  class="admin-menu hs-accordion-group flex w-full flex-col gap-1.5 p-4"
                >
                  <li class="group">
                    <RouterLink
                      class="flex items-center gap-x-3.5 py-3 px-4 text-sm text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                      to="/"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <rect width="7" height="7" x="3" y="3" rx="1"></rect>
                        <rect width="7" height="7" x="14" y="3" rx="1"></rect>
                        <rect width="7" height="7" x="14" y="14" rx="1"></rect>
                        <rect width="7" height="7" x="3" y="14" rx="1"></rect>
                      </svg>
                      Dashboard
                    </RouterLink>
                  </li>
                  <li class="">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleOrderFun"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <line x1="10" x2="21" y1="6" y2="6"></line>
                        <line x1="10" x2="21" y1="12" y2="12"></line>
                        <line x1="10" x2="21" y1="18" y2="18"></line>
                        <path d="M4 6h1v4"></path>
                        <path d="M4 10h2"></path>
                        <path d="M6 18H4c0-1 2-2 2-3s-1-1.5-2-1"></path>
                      </svg>
                      Orders
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-[height]"
                      :class="{ hidden: !toggleOrder }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/order-list"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Orders List
                          </RouterLink>
                        </li>
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/order-details"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Order Details
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                  <li class="">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleCustFun"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"
                        ></path>
                        <circle cx="9" cy="7" r="4"></circle>
                        <path d="M22 21v-2a4 4 0 0 0-3-3.87"></path>
                        <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                      </svg>
                      User
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-[height]"
                      :class="{ hidden: !toggleCust }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/user-list"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            User list
                          </RouterLink>
                        </li>

                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/add-user"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Add User
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                  <li class="">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleRestFun"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M18 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2Z"
                        ></path>
                        <path
                          d="m9 16 .348-.24c1.465-1.013 3.84-1.013 5.304 0L15 16"
                        ></path>
                        <path d="M8 7h.01"></path>
                        <path d="M16 7h.01"></path>
                        <path d="M12 7h.01"></path>
                        <path d="M12 11h.01"></path>
                        <path d="M16 11h.01"></path>
                        <path d="M8 11h.01"></path>
                        <path d="M10 22v-6.5m4 0V22"></path>
                      </svg>
                      Restaurants
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-[height]"
                      :class="{ hidden: !toggleRest }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/restaurant-list"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Restaurants List
                          </RouterLink>
                        </li>

                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/restaurant-add"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Add Restaurant
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                  <li class="">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleMenuFun"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 21a9 9 0 0 0 9-9H3a9 9 0 0 0 9 9Z"></path>
                        <path d="M7 21h10"></path>
                        <path d="M19.5 12 22 6"></path>
                        <path
                          d="M16.25 3c.27.1.8.53.75 1.36-.06.83-.93 1.2-1 2.02-.05.78.34 1.24.73 1.62"
                        ></path>
                        <path
                          d="M11.25 3c.27.1.8.53.74 1.36-.05.83-.93 1.2-.98 2.02-.06.78.33 1.24.72 1.62"
                        ></path>
                        <path
                          d="M6.25 3c.27.1.8.53.75 1.36-.06.83-.93 1.2-1 2.02-.05.78.34 1.24.74 1.62"
                        ></path>
                      </svg>
                      Menus
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-[height]"
                      :class="{ hidden: !toggleMenu }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/menu-list"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            menus List
                          </RouterLink>
                        </li>
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/menu-add"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Add menus
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                  <li class="">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleTableFun"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 21a9 9 0 0 0 9-9H3a9 9 0 0 0 9 9Z"></path>
                        <path d="M7 21h10"></path>
                        <path d="M19.5 12 22 6"></path>
                        <path
                          d="M16.25 3c.27.1.8.53.75 1.36-.06.83-.93 1.2-1 2.02-.05.78.34 1.24.73 1.62"
                        ></path>
                        <path
                          d="M11.25 3c.27.1.8.53.74 1.36-.05.83-.93 1.2-.98 2.02-.06.78.33 1.24.72 1.62"
                        ></path>
                        <path
                          d="M6.25 3c.27.1.8.53.75 1.36-.06.83-.93 1.2-1 2.02-.05.78.34 1.24.74 1.62"
                        ></path>
                      </svg>
                      Table
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-[height]"
                      :class="{ hidden: !toggleTable }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/add-table"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            add table
                          </RouterLink>
                        </li>
                        <li class="group"></li>
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/list-table"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            table list
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                  <li class="hs-accordion">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleBookingFun"
                    >
                      <!-- <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 21a9 9 0 0 0 9-9H3a9 9 0 0 0 9 9Z"></path>
                        <path d="M7 21h10"></path>
                        <path d="M19.5 12 22 6"></path>
                        <path
                          d="M16.25 3c.27.1.8.53.75 1.36-.06.83-.93 1.2-1 2.02-.05.78.34 1.24.73 1.62"
                        ></path>
                        <path
                          d="M11.25 3c.27.1.8.53.74 1.36-.05.83-.93 1.2-.98 2.02-.06.78.33 1.24.72 1.62"
                        ></path>
                        <path
                          d="M6.25 3c.27.1.8.53.75 1.36-.06.83-.93 1.2-1 2.02-.05.78.34 1.24.74 1.62"
                        ></path>
                      </svg> -->
                      Booking
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-all duration-300"
                      :class="{ hidden: !toggleBooking }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/bookings-list"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Booking List
                          </RouterLink>
                        </li>
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/booking-detail"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Booking Details
                          </RouterLink>
                        </li>
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/add-booking"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Add Booking
                          </RouterLink>
                        </li>
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/edit-booking"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Edit Booking
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                  <li class="">
                    <button
                      class="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-gray-100"
                      @click="toggleCategoryFun"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        height="20"
                        width="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"
                        ></path>
                        <circle cx="9" cy="7" r="4"></circle>
                        <path d="M22 21v-2a4 4 0 0 0-3-3.87"></path>
                        <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                      </svg>
                      Category
                      <svg
                        stroke="currentColor"
                        fill="none"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="ms-auto transition-all hs-accordion-active:rotate-180"
                        height="16"
                        width="16"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="m6 9 6 6 6-6"></path>
                      </svg>
                    </button>
                    <div
                      class="w-full overflow-hidden transition-[height]"
                      :class="{ hidden: !toggleCategory }"
                    >
                      <ul class="mt-2 space-y-2">
                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/category-list"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Category list
                          </RouterLink>
                        </li>

                        <li class="group">
                          <RouterLink
                            class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm font-medium text-default-700 rounded-md hover:bg-gray-100 group-[.active]:text-primary group-[.active]:bg-primary/10"
                            to="/add-category"
                          >
                            <svg
                              stroke="currentColor"
                              fill="none"
                              stroke-width="2"
                              viewBox="0 0 24 24"
                              stroke-linecap="round"
                              stroke-linejoin="round"
                              height="24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <circle cx="12.1" cy="12.1" r="1"></circle>
                            </svg>
                            Add Category
                          </RouterLink>
                        </li>
                      </ul>
                    </div>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div
        class="simplebar-track simplebar-horizontal"
        style="visibility: hidden"
      >
        <div
          class="simplebar-scrollbar"
          style="width: 0px; display: none"
        ></div>
      </div>
      <div
        class="simplebar-track simplebar-vertical"
        style="visibility: visible"
      >
        <div
          class="simplebar-scrollbar"
          style="
            height: 25px;
            transform: translate3d(0px, 0px, 0px);
            display: block;
          "
        ></div>
      </div>
    </div>
  </div>
</template>
<script setup>
import { ref } from "vue";
import { RouterLink } from "vue-router";
import { useGeneralStoreStore } from "@/stores/generalStore";
const store = useGeneralStoreStore();
import { storeToRefs } from "pinia";
const { toggleMenuMobileFun } = store;
const { toggleMenuMobile } = storeToRefs(store);
const toggleOrder = ref(false);
const toggleOrderFun = () => {
  toggleOrder.value = !toggleOrder.value;
};
const toggleCust = ref(false);
const toggleCustFun = () => {
  toggleCust.value = !toggleCust.value;
};
const toggleRest = ref(false);
const toggleRestFun = () => {
  toggleRest.value = !toggleRest.value;
};
const toggleMenu = ref(false);
const toggleMenuFun = () => {
  toggleMenu.value = !toggleMenu.value;
};
const toggleTable = ref(false);
const toggleTableFun = () => {
  toggleTable.value = !toggleTable.value;
};
const toggleBooking = ref(false);
const toggleBookingFun = () => {
  toggleBooking.value = !toggleBooking.value;
};
const toggleCategory = ref(false);
const toggleCategoryFun = () => {
  toggleCategory.value = !toggleCategory.value;
};
</script>
<style lang="scss" scoped></style>
