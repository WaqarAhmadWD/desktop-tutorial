            ...
            <div>
              ...
              >
                <option
                  ...
                  :key="option"
                  :value="option"
                  :selected="singleTableLocal.status == option"
                >
                  {{ option }}
                </option>
              </select>
...
const singleTableLocal = ref({ tableNumber: "", status: "", capacity: 0 });
import { useRestaurantAdminStore } from "@/stores/restaurantAdmin";
import { storeToRefs } from "pinia";
const restStore = useRestaurantAdminStore();
const { fetchSingleTable, editTable } = restStore;
const { singleTable, loading } = storeToRefs(restStore);
const options = ["available", "reserved", "occupied"];
onMounted(async () => {
  await fetchSingleTable(props.id);
  singleTableLocal.value = await singleTable.value;
  console.log(singleTableLocal.value);
});
const submit = () => {
  editTable(singleTableLocal.value);
};
</script>
